...
def test_bloodied_stalker_surfaces_cure_wounds_on_aelric(mountin_pass_win):
    ...
    aelric_idx = next(
        ...
        if isinstance(win.tabs.widget(i), NPCTab) and win.tabs.widget(i).npc_state.slug == "aelric-frostweaver"
    )
    assert aelric_idx in watch_buckets
    suggestions = watch_buckets[aelric_idx]
...
def test_recovered_target_prunes_watch_suggestion(mountin_pass_win):
    ...
    aelric_idx = next(
        i for i in range(win.tabs.count())
        if win.tabs.widget(i).npc_state.slug == "aelric-frostweaver"
    )
    # Confirm a watch suggestion exists
    assert any(s.action_name == "cure_wounds" for s in win._watch_suggestions.get(aelric_idx, []))
    ...
    assert not any(s.action_name == "cure_wounds" for s in win._watch_suggestions.get(aelric_idx, []))
