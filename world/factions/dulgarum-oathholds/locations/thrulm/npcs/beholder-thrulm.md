...
1. If **Disintegration Ray** is USED, roll `roll_dice(1, 6)` — recovers on 5–6. **DR is a ranged ATTACK ROLL (+6 to hit, range 120 ft), NOT a Dex saving throw** — on a hit, full 10d8 force damage applies (no half-damage on a miss). Contrast with Void Ray (legendary) which is a DC 16 Dex save. **DISINTEGRATION RULING — announce immediately on a 0-HP hit:** A creature reduced to 0 HP by DR is reduced to dust; Lay on Hands requires touch and cannot revive a disintegrated creature (FI-NEW-FC4-LOH). Declare this *before* Sabriel's next action so she doesn't waste LoH. (True Resurrection / Wish only — party has neither at L5.)
...
- **Compel Thrall targeting (FI-20):** When using `compel_thrall` bonus action to direct a dominated thrall, aim them at the **lowest-AC PC** (typically Marwen, AC 15 — +4 to-hit gives ~45% hit rate vs ~30% against Sabriel AC 19). Directing thralls at the divine martial is a near-wasted command; redirect to the caster or whoever just burned a resource. Do NOT compel toward a PC who is already focused on a thrall — that wastes the beholder's bonus action on redundant direction. **Re-target on death (FI-FC27-A):** If the primary target (lowest-AC PC) was killed or disintegrated earlier this same turn, OR is currently at 0 HP from any prior round, immediately update to the next-lowest-AC living PC — do not direct thralls toward a dead or downed creature's last position. (FIX-BEL3-2-A: extended to cover multi-round LoH revival cycles where primary target oscillates 0→revived→0.) **Skip if no live thralls (MQ-BEL25-A):** Do not use `compel_thrall` if all thralls are dead or at 0 HP — there is no valid charmed target and the bonus action is wasted. Use `shrine_drift` instead.
