...
- **Lair actions:** at init 20, prefer `void_eruption` if 2+ PCs near the altar; otherwise `unstable_ground` on a melee threat; `manifest_thralls` only if thralls are alive AND positioned usefully. **Lair rotation (FI-129):** Do not repeat the same lair action more than 2 consecutive rounds — rotate even if the optimal choice would repeat. Skip `unstable_ground` if the target is already prone OR unconscious (wasted action). Use `manifest_thralls` if thralls are still up and repositioning them would open a flank.
