...
- **Multi-rager Berserk stagger:** When 2+ ragers all have Berserk available and can each reach 2+ PCs simultaneously (common in R1 when the party is clustered), stagger Berserk across turns — only ONE rager uses Berserk per round. Rule: **the highest-initiative rager that hasn't yet used Berserk this combat fires it; others use Multiattack and hold their Berserk.** Firing 3× Berserk in a single round (9 attack rolls in one turn sequence) produces an unintended nova burst and eliminates the meaningful decision of "when to spend the recharge." *(FI-SR5-51-B: 7th-cycle SR5 run — all 3 ragers fired Berserk R1, no stagger guidance existed; added this note.)*
