...
Same contract as [`foundry-markers.md`](foundry-markers.md): drop a `Marker · 1` on the stair
...
- **The token document is destroyed and rebuilt, but the id survives.** Cross-scene teleport
  ...
  See [`playbooks/foundry-npc-ties.md`](../playbooks/foundry-npc-ties.md).
...
- [`foundry-markers.md`](foundry-markers.md) — markers, the centre-on-the-marker contract, canvas
  vs image coordinates, the eye hotspot icon.
- [`forge-st.md`](forge-st.md) — scene ids, folder trees, token conventions.
