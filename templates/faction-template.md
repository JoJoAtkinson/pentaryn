# [Faction Name]

**Tags:** `#faction` `#organization` `#guild` `#cult`  
**Created:** YYYY-MM-DD  
**Last Modified:** YYYY-MM-DD  
**Status:** Active/Defunct/Hidden

## Overview

| Attribute | Value |
|-----------|-------|
| **Type** | Guild/Religious Order/Thieves' Guild/Military/Government/etc. |
| **Size** | Small/Medium/Large/Regional/Continental |
| **Alignment** | Typical alignment |
| **Headquarters** | [Location](locations/location-name.md) |
| **Founded** | Year/Era |

## Symbol & Colors

**Symbol:** Description or image of the faction's emblem  
**Colors:** Primary and secondary colors  
**Motto/Slogan:** "Faction motto"

## Mission & Goals

### Public Goals
What the faction claims to stand for publicly.

### True Goals
What the faction actually works toward (if different from public goals).

### Methods
How they pursue their goals.

## History

### Foundation
How and why the faction was created.

### Major Events
- **Event 1 (Year):** What happened and impact
- **Event 2 (Year):** Description
- 

### Current Situation
Where the faction stands today.

## Leadership Structure

### Hierarchy
Description of the organizational structure.

```
Leader/High Council
    ↓
Regional Leaders
    ↓
Local Chapters
    ↓
Regular Members
```

### Key Leaders

#### [Leader Name](../characters/npcs/leader-name.md)
- **Title:** Supreme Leader/Guildmaster/etc.
- **Role:** What they do
- **Description:** Brief description

#### [Lieutenant Name](../characters/npcs/lieutenant-name.md)
- **Title:** 
- **Role:** 
- **Description:** 

## Membership

### Requirements
How to join the faction.

### Ranks & Titles
1. **Initiate:** Entry level
2. **Member:** Full member
3. **Veteran:** Experienced member
4. **Officer:** Leadership position
5. **Master:** Highest rank

### Benefits
What members gain from belonging.

### Obligations
What is expected of members.

## Resources & Assets

### Financial
- **Treasury:** Wealth level
- **Income Sources:** Where money comes from

### Military
- **Forces:** Number and type of fighters
- **Equipment:** Weapons and armor available

### Magical
- **Spellcasters:** Number and power level
- **Artifacts:** [Important Items](../items/artifacts/item-name.md)

### Political
- **Influence:** Areas of political power
- **Alliances:** Allied factions

## Locations

### [Headquarters](locations/headquarters-name.md)
Description of main base.

### [Outpost Name](locations/outpost-name.md)
Description of secondary location.

### Territory
Regions where the faction has presence or control.

## Allies & Enemies

### Allies
- **[Faction Name](../faction-name/_overview.md):** Nature of alliance
- 

### Rivals
- **[Faction Name](../faction-name/_overview.md):** Source of rivalry
- 

### Enemies
- **[Faction Name](../faction-name/_overview.md):** Reason for enmity
- 

## Reputation

### Among Common Folk
How ordinary people view this faction.

### Among Nobility
How the ruling class views this faction.

### Among Other Factions
General reputation in the power structure.

## Activities & Operations

### Public Activities
What the faction does openly.

### Secret Operations
Hidden activities (if any).

### Current Projects
Active initiatives or campaigns.

## Quests & Involvement

### Quests Offered
- [Quest Name](../quests/active/quest-name.md)
- 

### Related Quests
- [Quest Name](../quests/active/quest-name.md)
- 

## Player Faction Reputation

Track party standing with this faction.

| Character | Rank | Reputation | Notes |
|-----------|------|------------|-------|
| [Name](../characters/player-characters/name.md) | | Friendly/Neutral/Hostile | |
| | | | |

### Reputation Effects
- **Friendly (25+):** Benefits
- **Neutral (0-24):** Standard treatment
- **Hostile (-25 to -1):** Negative consequences

## Secrets & Mysteries

Information only the DM should know about this faction.

## Plot Hooks

1. **Hook Title:** Description of story opportunity
2. 
3. 

## DM Notes

### Using This Faction
Tips for incorporating this faction into your campaign.

### Future Plans
Long-term story arcs involving this faction.

---

## Related Links

- [Leader](../characters/npcs/leader-name.md)
- [Headquarters](../locations/headquarters-name.md)
- [Related Quest](../quests/active/quest-name.md)
- [Allied Faction](faction-name.md)
- [Enemy Faction](faction-name.md)
