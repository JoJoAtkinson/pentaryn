# [Location Name]

**Tags:** `#location` `#city` or `#dungeon` or `#region`  
**Created:** YYYY-MM-DD  
**Last Modified:** YYYY-MM-DD

## Overview

| Attribute | Value |
|-----------|-------|
| **Type** | City/Town/Village/Dungeon/Wilderness/etc. |
| **Region** | |
| **Population** | |
| **Government** | |
| **Ruler** | |

## Description

> Read-aloud text for when players first arrive or see this location.

Detailed description of the location. What does it look like? What's the atmosphere? What makes it unique?

## Questions to Ask

When you're considering creating this place, ask the following to help shape its role in the world and stories:

- **Why does this place exist?**
- **Why is it still here?**
- **Why hasn’t it been destroyed yet?**
- **Who wants control of it right now?**
- **What terrible problem is hidden underneath?**
  - Use this to seed hidden hooks, secrets, or ongoing threats that can drive adventures.

## History

Background and significant historical events related to this location.

## Geography

### Climate
Description of weather and seasons.

### Terrain
Physical features and landscape.

### Notable Features
- Feature 1: Description
- Feature 2: Description

## Points of Interest

### 1. [Place Name]
**Type:** Inn/Shop/Temple/etc.  
**Description:** What is this place?  
**NPCs:** Who can be found here?  
**Services:** What can players do here?

### 2. [Place Name]
**Type:**  
**Description:**  
**NPCs:**  
**Services:**

## NPCs & Inhabitants

### Key NPCs
- **[NPC Name](../characters/npcs/npc-name.md):** Role and description
- 

### Demographics
Description of the general population and their occupations.

## Factions & Organizations

- **[Faction Name](../world/factions/faction-name.md):** Influence and presence in this location
- 

## Economy & Trade

- **Main Exports:** 
- **Main Imports:** 
- **Currency:** 
- **Trade Routes:** 

## Rumors & Hooks

1. **Rumor/Hook Title:** Description of plot hook or rumor players might hear
2. 
3. 

## Quests Available Here

- [Quest Name](../quests/active/quest-name.md)
- 

## Encounters

### Random Encounters
- 1d6 = 1-2: Encounter type
- 1d6 = 3-4: Encounter type
- 1d6 = 5-6: Encounter type

### Scheduled Encounters
Events that happen at specific times or conditions.

## Maps & Layout

Description of the layout. Add map image if available:
![Map](path-to-map-image.png)

### Areas/Rooms
1. **Area 1:** Description
2. **Area 2:** Description

## Shops & Services

### [Shop Name]
- **Proprietor:** [NPC Name]
- **Goods:**
  - Item 1: Price
  - Item 2: Price

## Dangers & Hazards

Environmental hazards, threats, or dangerous creatures in the area.

## DM Notes

Information only for the DM. Secret rooms, hidden treasures, plot developments.

---

## Related Links

- [Parent Region](../world/locations/region-name.md)
- [Nearby Location](location-name.md)
- [Related Quest](../quests/active/quest-name.md)
