# Session [Number] Planning - [Date]

**Tags:** `#session` `#planning`  
**Planned Date:** YYYY-MM-DD  
**Session Number:** XX  
**Expected Duration:** X hours

## Session Goals

What do you want to accomplish this session?

1. Goal 1
2. Goal 2
3. Goal 3

## Last Session Recap

Quick summary of what happened last session and where we left off.

- Location: [Location Name](../world/locations/location-name.md)
- Situation: 
- Cliffhanger: 

## Planned Story Beats

### Opening Scene
How the session will start.

**Location:** [Location](../world/locations/location-name.md)  
**Setup:** 

> Read-aloud opening text

### Beat 1: [Title]
**Location:**  
**Purpose:** What this scene accomplishes  
**NPCs:** [List relevant NPCs](../characters/npcs/npc-name.md)  
**Description:** 

### Beat 2: [Title]
**Location:**  
**Purpose:**  
**NPCs:**  
**Description:** 

### Beat 3: [Title]
**Location:**  
**Purpose:**  
**NPCs:**  
**Description:** 

### Potential Climax
Likely high point of the session.

### Ending/Cliffhanger
Where you'd like to end if possible.

## Prepared Encounters

### Combat Encounter 1: [Name]
**Location:** [Location](../world/locations/location-name.md)  
**Trigger:** What causes this encounter  
**Enemies:**
- X × [Creature](../creatures/monsters/creature-name.md)
- X × [Creature](../creatures/monsters/creature-name.md)

**Tactics:** How enemies fight  
**Victory Condition:**  
**Defeat Condition:**  
**XP:** XXX

### Combat Encounter 2: [Name]
**Location:**  
**Trigger:**  
**Enemies:**  
**Tactics:**  

### Skill Challenge: [Name]
**Setup:** Description of the challenge  
**Skills Involved:** List of relevant skills  
**DCs:** 
- Easy: DC 10
- Medium: DC 15
- Hard: DC 20

**Success:** What happens on success  
**Failure:** Consequences of failure

### Social Encounter: [Name]
**NPC(s):** [Name](../characters/npcs/npc-name.md)  
**Goal:** What the NPC wants  
**Information Available:** What players can learn  
**Persuasion DCs:** 
**Potential Outcomes:** 

## NPCs This Session

### [NPC Name](../characters/npcs/npc-name.md)
- **Role:** How they're involved
- **Goal:** What they want
- **Key Info:** Important facts
- **Voice/Mannerism:** Roleplay notes

### [NPC Name](../characters/npcs/npc-name.md)
- **Role:**
- **Goal:**
- **Key Info:**
- **Voice/Mannerism:**

## Locations Used

### [Location Name](../world/locations/location-name.md)
- **Key Features:** Important details to mention
- **Atmosphere:** Mood and description
- **Secrets:** Hidden things

## Loot & Rewards

### Treasure
- Gold: XXX gp
- Items: 
  - [Item Name](../items/magic-items/item-name.md)
  - 

### Quest Rewards
- [Quest Name](../quests/active/quest-name.md): Rewards for completion

### XP Budget
- Total XP available: XXX
- Per player: XXX

## Player Character Hooks

### [PC Name](../characters/player-characters/pc-name.md)
Specific content or hooks for this character this session.

### [PC Name](../characters/player-characters/pc-name.md)
Content for this character...

## Flexibility Points

Places where the session can adapt based on player choices:

1. **If players do X:** How to adapt
2. **If players do Y:** Alternative path
3. **If players ignore Z:** Contingency

## Foreshadowing & Future Setup

Elements to plant for future sessions:

- Element 1: Description
- Element 2: Description

## Props & Materials

### Physical Props
- [ ] Maps (list which ones)
- [ ] Miniatures needed
- [ ] Handouts
- [ ] Music playlist

### Digital Prep
- [ ] NPC stat blocks ready
- [ ] Monster tokens/images
- [ ] Item cards
- [ ] Location images/maps

### Reference Materials
- [ ] Rules to review
- [ ] Previous session notes
- [ ] Quest details

## Random Tables

### Random Events (1d6)
1. Event 1
2. Event 2
3. Event 3
4. Event 4
5. Event 5
6. Event 6

### Random NPCs
Quick NPCs if improvisation is needed:
1. Name, profession, quirk
2. Name, profession, quirk
3. Name, profession, quirk

## Pacing Guide

Rough time estimates to keep session moving:

| Time | Activity |
|------|----------|
| 0:00-0:15 | Recap and opening |
| 0:15-1:00 | Beat 1 |
| 1:00-1:45 | Beat 2 / Encounter |
| 1:45-2:00 | Break |
| 2:00-2:45 | Beat 3 |
| 2:45-3:30 | Climax/Resolution |
| 3:30-4:00 | Wrap up and cliffhanger |

## DM Reminders

- [ ] Track spell slots
- [ ] Track HP
- [ ] Track resources (arrows, potions, etc.)
- [ ] Award inspiration
- [ ] Note important developments for next session

## Backup Plans

### If Session Goes Short
Additional content to extend if needed:
- 

### If Session Goes Long
What can be cut or moved to next session:
- 

### If Players Go Off-Script
How to handle unexpected directions:
- 

## Post-Session Notes

Fill this out after the session:

### What Actually Happened

### What Worked Well

### What To Improve

### For Next Session

---

## Related Links

- [Previous Session Notes](../notes/session-XX-YYYY-MM-DD.md)
- [Active Quests](../../quests/active/)
- [Campaign Overview](../../README.md)
