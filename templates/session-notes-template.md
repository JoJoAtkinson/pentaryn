# Session [Number] - [Date]

**Tags:** `#session`  
**Date Played:** YYYY-MM-DD  
**Session Number:** XX  
**In-Game Date:** 

## Party Members Present

- [Character Name](../characters/player-characters/character-name.md) - Player Name
- 
- 

## Session Summary

Brief overview of what happened this session (2-3 sentences).

## Detailed Notes

### Part 1: [Title/Scene Name]

**Location:** [Location Name](../../world/factions/region-name/locations/location-name.md)

What happened in this part of the session. Include:
- Dialogue and interactions
- Decisions made
- Rolls and outcomes
- Combat encounters

### Part 2: [Title/Scene Name]

**Location:** 

Continue the narrative...

## Combat Encounters

### Encounter 1: [Name]
- **Location:** 
- **Enemies:** 
  - 3x Goblin
  - 1x Hobgoblin
- **Outcome:** Victory/Defeat/Flee/Negotiation
- **XP Awarded:** 
- **Casualties:** 
- **Notable Moments:** 

## NPCs Met

- **[NPC Name](../characters/npcs/npc-name.md):** How the party interacted with them
- 

## Locations Visited

- **[Location Name](../../world/factions/region-name/locations/location-name.md):** What happened there
- 

## Quests Updated

### New Quests
- **[Quest Name](../quests/active/quest-name.md):** How it was obtained

### Progressed Quests
- **[Quest Name](../quests/active/quest-name.md):** Progress made

### Completed Quests
- **[Quest Name](../quests/completed/quest-name.md):** How it was completed
- **Rewards:** 

## Loot & Rewards

### Items Found
- Item 1 (assigned to Character Name)
- Item 2 (party inventory)

### Gold & Currency
- Gold pieces found: 
- Total party gold: 

### Experience Points
- XP this session: 
- Total XP: 
- Next level: 

## Important Discoveries

- Discovery 1: Description and implications
- 

## Character Moments

Noteworthy roleplay, character development, or funny moments.

- **[Character Name]:** What they did/said
- 

## Cliffhanger/Where We Left Off

Where the session ended and what's immediately next.

## Next Session Planning

### Preparation Needed
- [ ] Prepare encounter for X
- [ ] Create NPC stats for Y
- [ ] Draw map of Z

### Likely Direction
Where the party is likely to go next.

### Loose Ends
Unresolved plot threads or things the party mentioned wanting to do.

## DM Notes

Private notes about how the session went, what worked, what didn't, future plans.

---

## Related Links

- [Previous Session](session-XX-YYYY-MM-DD.md)
- [Next Session](session-XX-YYYY-MM-DD.md)
- [Campaign Home](../../README.md)
